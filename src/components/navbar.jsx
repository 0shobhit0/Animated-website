...
const Navbar = () => {
  ...
  return (
    <div className='navbar fixed top-0 left-0 z-[999] w-full px-[2vh] py-[4vh] flex justify-between items-center'>
      <div className='logo'>
        <svg
          width='72'
          height='30'
      ...
      <div className='lg:hidden absolute top-0 left-0'>
        <button onClick={toggleSidebar}>
          {showSidebar ? <FaTimes /> : <FaBars />} {/* Toggle between hamburger and close icons */}
        </button>
      </div>
      <div className={`links flex gap-6 lg:gap-10 ${showSidebar ? 'flex-col' : 'hidden'} lg:flex`}>
        {["Services", "Our Work", "About Us", "Insights", "Contact"].map(
          (item, index) => (
            <a
              key={index}
              className={`text-lg capitalize font-light ${
                index === 4 && "lg:ml-12 xl:ml-32"
              }`}
            >
              {item}
            </a>
